...
class LinkValidator {
  ...
  buildFileStructure() {
    ...
    const astroFiles = [
      ...
      "src/pages/docs/specifications/activitystreams-vocabulary-llm-specification-guide.astro",
      "src/pages/docs/specifications/fedify-cli-llm-specification-guide.astro",
    ...
    console.log(
      `📊 Found ${this.astroFiles.length} Astro files and ${this.markdownFiles.length} Markdown files`,
    );
  }
  ...
  extractLinksFromFile(filePath) {
    ...
    try {
      ...
      for (const match of content.matchAll(astroHrefRegex)) {
        links.push({
          text: "Astro href",
          url: match[1],
          type: "astro",
  ...
  generateReport() {
    ...
    // Summary
    const totalLinks =
      this.results.navigation.length +
    ...
    console.log(
      `Success rate: ${(((totalLinks - brokenLinks.length) / totalLinks) * 100).toFixed(1)}%`,
    );
