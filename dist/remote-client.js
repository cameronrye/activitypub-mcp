...
const DomainSchema = z
    ...
    !domain.startsWith(".") &&
    !domain.endsWith("."), "Invalid domain format");
...
export class RemoteActivityPubClient {
     ...
     */
    async fetchRemoteActor(identifier) {
        ...
        try {
            ...
            throw new Error(`Failed to fetch actor ${identifier}: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    }
    /**
     * Fetch actor's outbox (timeline/posts)
     */
    async fetchActorOutbox(identifier, limit = 20) {
        ...
        return await this.fetchWithRetry(outboxUrl.toString(), {
            headers: {
     ...
     */
    async fetchActorFollowers(identifier, limit = 20) {
        const actor = await this.fetchRemoteActor(identifier);
        if (!actor.followers) {
        ...
        return await this.fetchWithRetry(followersUrl.toString(), {
            headers: {
                Accept: 'application/activity+json, application/ld+json; profile="https://www.w3.org/ns/activitystreams"',
                "User-Agent": USER_AGENT,
            },
     ...
     */
    async fetchActorFollowing(identifier, limit = 20) {
        const actor = await this.fetchRemoteActor(identifier);
        if (!actor.following) {
        ...
        return await this.fetchWithRetry(followingUrl.toString(), {
            headers: {
                Accept: 'application/activity+json, application/ld+json; profile="https://www.w3.org/ns/activitystreams"',
                "User-Agent": USER_AGENT,
            },
     ...
     */
    async fetchObject(objectUrl) {
        logger.info("Fetching ActivityPub object", { url: objectUrl });
        return await this.fetchWithRetry(objectUrl, {
            headers: {
                Accept: 'application/activity+json, application/ld+json; profile="https://www.w3.org/ns/activitystreams"',
                "User-Agent": USER_AGENT,
            },
     ...
     */
    async searchInstance(domain, query, type = "accounts") {
        // Validate domain input
        const validDomain = DomainSchema.parse(domain);
        ...
        return await this.fetchWithRetry(searchUrl, {
            headers: {
                Accept: "application/json",
                "User-Agent": USER_AGENT,
            },
     ...
     */
    transformInstanceInfo(domain, data, endpoint) {
        const base = { domain };
        ...
        if (endpoint.includes("/api/v1/instance")) {
            ...
            return {
                ...base,
                software: typeof dataObj.version === "string" &&
                    dataObj.version.includes("Pleroma")
                    ? "pleroma"
                    : "mastodon",
                version: typeof dataObj.version === "string" ? dataObj.version : undefined,
                description: typeof dataObj.description === "string"
                    ? dataObj.description
                    : undefined,
                languages: Array.isArray(dataObj.languages)
                    ? dataObj.languages
                    : undefined,
                registrations: typeof dataObj.registrations === "boolean"
                    ? dataObj.registrations
                    : undefined,
                approval_required: typeof dataObj.approval_required === "boolean"
                    ? dataObj.approval_required
                    : undefined,
                contact_account: typeof dataObj.contact_account === "object"
                    ? dataObj.contact_account
                    : undefined,
                stats: typeof dataObj.stats === "object"
                    ? dataObj.stats
                    : undefined,
            };
        }
        ...
        if (endpoint.includes("/nodeinfo")) {
            // NodeInfo format
            const software = typeof dataObj.software === "object" && dataObj.software !== null
                ? dataObj.software
                : {};
            const metadata = typeof dataObj.metadata === "object" && dataObj.metadata !== null
                ? dataObj.metadata
                : {};
            return {
                ...base,
                ...
                description: typeof metadata.nodeDescription === "string"
