...
class HealthChecker {
     ...
     */
    async performHealthCheck(includeMetrics = false) {
        ...
        if (includeMetrics) {
            ...
            result.metrics = {
                requests: {
                    ...
                    errorRate: perfMetrics.requestCount > 0
     ...
     */
    async checkEnvironmentVariables(checks) {
        const startTime = Date.now();
     ...
     */
    async checkMCPServerStatus(checks) {
        const startTime = Date.now();
        try {
     ...
     */
    async checkRateLimiting(checks) {
        const startTime = Date.now();
        try {
