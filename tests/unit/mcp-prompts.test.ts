...
describe("MCP Prompts", () => {
  ...
  let registeredPrompts: Map<string, { handler: (args: Record<string, unknown>) => { messages: unknown[] }; config: unknown }>;
  ...
  beforeEach(() => {
    ...
    mcpServer = {
      registerPrompt: vi.fn((name: string, config: unknown, handler: (args: Record<string, unknown>) => { messages: unknown[] }) => {
  ...
  describe("registerPrompts", () => {
    it("should register all expected prompts", () => {
      ...
      for (const promptName of expectedPrompts) {
        expect(registeredPrompts.has(promptName), `Prompt ${promptName} should be registered`).toBe(true);
      }
    });
  });
  ...
  describe("explore-fediverse prompt", () => {
    it("should generate exploration message with interests", () => {
      ...
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("technology, programming");
    ...
    it("should include instance type preference when specified", () => {
      ...
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("pixelfed");
    });
    ...
    it("should work without instance type", () => {
      ...
      expect((result.messages[0] as { content: { text: string } }).content.text).not.toContain("I prefer");
    });
  });
  ...
  describe("compare-instances prompt", () => {
    it("should generate comparison message with instances", () => {
      ...
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("mastodon.social, fosstodon.org");
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("compare");
    });

    it("should include criteria when specified", () => {
      const prompt = registeredPrompts.get("compare-instances");
      const result = prompt!.handler({ instances: "mastodon.social", criteria: "moderation, privacy" });

      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("moderation, privacy");
    });
  });
  ...
  describe("discover-content prompt", () => {
    it("should generate content discovery message", () => {
      ...
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("rust, webdev");
    });
    ...
    it("should customize message for people content type", () => {
      ...
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("people");
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("accounts to follow");
    });
    ...
    it("should customize message for hashtags content type", () => {
      ...
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("hashtags");
    });
    ...
    it("should customize message for instances content type", () => {
      ...
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("instances to explore");
    });
  });
  ...
  describe("compare-accounts prompt", () => {
    it("should generate account comparison message", () => {
      ...
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("user1@mastodon.social, user2@fosstodon.org");
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("posting frequency");
    });

    it("should include aspects when specified", () => {
      const prompt = registeredPrompts.get("compare-accounts");
      const result = prompt!.handler({ accounts: "user@example.social", aspects: "engagement, topics" });

      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("engagement, topics");
    });
  });
  ...
  describe("analyze-user-activity prompt", () => {
    it("should generate analysis message with default depth", () => {
      ...
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("testuser@mastodon.social");
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("Analyze their recent 20 posts");
    });
    ...
    it("should customize for quick analysis", () => {
      ...
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("10 most recent posts");
    });
    ...
    it("should customize for comprehensive analysis", () => {
      ...
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("up to 50 posts");
    });
  });
  ...
  describe("find-experts prompt", () => {
    it("should generate expert finding message", () => {
      ...
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("machine learning");
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("experts");
    });

    it("should include specific instances when provided", () => {
      const prompt = registeredPrompts.get("find-experts");
      const result = prompt!.handler({ topic: "climate science", instances: "mastodon.social, scholar.social" });

      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("mastodon.social, scholar.social");
    });
  });
  ...
  describe("summarize-trending prompt", () => {
    it("should generate trending summary message", () => {
      ...
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("trending");
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("mastodon.social");
    });
    ...
    it("should use specified instances", () => {
      ...
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("fosstodon.org, techhub.social");
    });
  ...
  describe("content-strategy prompt", () => {
    it("should generate content strategy message", () => {
      ...
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("javascript, webdev");
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("content strategy");
    });
    ...
    it("should include target audience", () => {
      ...
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("beginners");
    });
  ...
  describe("community-health prompt", () => {
    it("should generate community health analysis message", () => {
      ...
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("mastodon.social");
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("community health");
    });
    ...
    it("should include specific concerns", () => {
      ...
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("spam, moderation");
    });
  });
  ...
  describe("migration-helper prompt", () => {
    it("should generate migration helper message", () => {
      ...
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("migration");
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("privacy, moderation");
    });
    ...
    it("should include current and target instances", () => {
      ...
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("mastodon.social");
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("fosstodon.org");
    });
    ...
    it("should handle new user without current instance", () => {
      ...
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("New to Fediverse");
    });
  });
  ...
  describe("thread-composer prompt", () => {
    it("should generate thread composer message", () => {
      ...
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("Introduction to TypeScript");
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("type safety, interfaces, generics");
    });
    ...
    it("should customize for different tones", () => {
      ...
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("storytelling");
    });
    ...
    it("should handle content warning", () => {
      ...
      expect((result.messages[0] as { content: { text: string } }).content.text).toContain("contains discussion of difficult topics");
    });
    ...
    it("should respect includeHashtags flag", () => {
      ...
      expect((withHashtags.messages[0] as { content: { text: string } }).content.text).toContain("Hashtag Research");
      ...
      expect((withoutHashtags.messages[0] as { content: { text: string } }).content.text).not.toContain("Hashtag Research");
    });
