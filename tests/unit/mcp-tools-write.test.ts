...
vi.mock("../../src/auth/index.js", () => ({
  accountManager: {
    listAccounts: vi.fn().mockReturnValue([
      { id: "1", username: "testuser", instance: "example.social", isActive: true, label: "Test", scopes: ["read", "write"] },
    ]),
    getActiveAccount: vi.fn().mockReturnValue({ id: "1", username: "testuser", instance: "example.social" }),
    getAccount: vi.fn().mockReturnValue({ id: "1", username: "testuser", instance: "example.social" }),
  ...
  authenticatedClient: {
    ...
    getWriteStatus: vi.fn().mockReturnValue({
      enabled: true,
      activeAccount: { id: "1", username: "testuser", instance: "example.social", isActive: true, label: "Test User", scopes: ["read", "write"] },
    }),
    ...
    lookupAccount: vi.fn().mockResolvedValue({ id: "target-123", acct: "targetuser@example.social" }),
...
describe("MCP Write Tools", () => {
  ...
  describe("list-accounts tool", () => {
    it("should list configured accounts", async () => {
      ...
      expect((result as { content: { text: string }[] }).content[0].text).toContain("Configured Accounts");
    ...
    it("should show no accounts message when empty", async () => {
      ...
      expect((result as { content: { text: string }[] }).content[0].text).toContain("No Accounts Configured");
    });
  });
  ...
  describe("switch-account tool", () => {
    it("should switch active account", async () => {
      ...
      expect((result as { content: { text: string }[] }).content[0].text).toContain("Account Switched");
    ...
    it("should handle account not found", async () => {
      ...
      expect((result as { content: { text: string }[] }).content[0].text).toContain("Account Not Found");
    });
  });
  ...
  describe("verify-account tool", () => {
    it("should verify credentials", async () => {
      ...
      expect((result as { content: { text: string }[] }).content[0].text).toContain("Account Verified");
    ...
    it("should handle verification failure", async () => {
      ...
      expect((result as { isError: boolean }).isError).toBe(true);
      expect((result as { content: { text: string }[] }).content[0].text).toContain("Verification Failed");
    });
  });
  ...
  describe("unboost-post tool", () => {
    it("should unboost a post", async () => {
      ...
      expect((result as { content: { text: string }[] }).content[0].text).toContain("Boost Removed");
  ...
  describe("favourite-post tool", () => {
    it("should favourite a post", async () => {
      ...
      expect((result as { content: { text: string }[] }).content[0].text).toContain("Post Favourited");
  ...
  describe("unfavourite-post tool", () => {
    it("should unfavourite a post", async () => {
      ...
      expect((result as { content: { text: string }[] }).content[0].text).toContain("Favourite Removed");
  ...
  describe("bookmark-post tool", () => {
    it("should bookmark a post", async () => {
      ...
      expect((result as { content: { text: string }[] }).content[0].text).toContain("Post Bookmarked");
  ...
  describe("unbookmark-post tool", () => {
    it("should unbookmark a post", async () => {
      ...
      expect((result as { content: { text: string }[] }).content[0].text).toContain("Bookmark Removed");
  ...
  describe("follow-account tool", () => {
    it("should follow an account", async () => {
      ...
      expect((result as { content: { text: string }[] }).content[0].text).toContain("Now following");
    ...
    it("should handle follow request pending", async () => {
      (authenticatedClient.followAccount as Mock).mockResolvedValueOnce({ following: false, requested: true });

      const tool = registeredTools.get("follow-account");
      const result = await tool!.handler({ acct: "targetuser@example.social" });

      expect((result as { content: { text: string }[] }).content[0].text).toContain("Follow request sent");
    });
  });
  ...
  describe("get-home-timeline tool", () => {
    it("should get home timeline", async () => {
      ...
      expect((result as { content: { text: string }[] }).content[0].text).toContain("Home Timeline");
  ...
  describe("get-notifications tool", () => {
    it("should get notifications", async () => {
      ...
      expect((result as { content: { text: string }[] }).content[0].text).toContain("Notifications");
  ...
  describe("vote-on-poll tool", () => {
    it("should vote on a poll", async () => {
      ...
      expect((result as { content: { text: string }[] }).content[0].text).toContain("Vote Recorded");
  ...
  describe("upload-media tool", () => {
    it("should handle file not found gracefully", async () => {
      ...
      expect((result as { isError: boolean }).isError).toBe(true);
      expect((result as { content: { text: string }[] }).content[0].text).toContain("Failed to upload media");
    });
  });
  ...
  describe("get-scheduled-posts tool", () => {
    it("should get scheduled posts (empty)", async () => {
      ...
      expect((result as { content: { text: string }[] }).content[0].text).toContain("Scheduled Posts");
  ...
  describe("cancel-scheduled-post tool", () => {
    it("should cancel a scheduled post", async () => {
      ...
      expect((result as { content: { text: string }[] }).content[0].text).toContain("Scheduled Post Canceled");
  ...
  describe("update-scheduled-post tool", () => {
    it("should update a scheduled post", async () => {
      ...
      const result = await tool!.handler({ scheduledId: "scheduled-1", scheduledAt: "2024-12-26T10:00:00Z" });

      expect((result as { content: { text: string }[] }).content[0].text).toContain("Scheduled Post Updated");
