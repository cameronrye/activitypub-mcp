...
vi.mock("../../src/remote-client.js", () => ({
  remoteClient: {
    fetchActorOutboxPaginated: vi.fn().mockResolvedValue({
      items: [
        { id: "1", content: "<p>Test post</p>", published: "2024-01-01", attributedTo: "user@example.social" },
        { id: "2", content: "<p>Another post</p>", published: "2024-01-02", attributedTo: "user@example.social" },
      ],
      totalItems: 2,
      hasMore: false,
    }),
...
describe("MCP Export Tools", () => {
  ...
  describe("export-timeline tool", () => {
    it("should export timeline in JSON format", async () => {
      ...
      expect((result as { content: { text: string }[] }).content[0].text).toContain("Timeline Export");
    ...
    it("should handle export errors gracefully", async () => {
      (remoteClient.fetchActorOutboxPaginated as Mock).mockRejectedValueOnce(new Error("Export failed"));
  ...
  describe("export-thread tool", () => {
    it("should export thread in JSON format", async () => {
      ...
      expect((result as { content: { text: string }[] }).content[0].text).toContain("Thread Export");
  ...
  describe("export-account-info tool", () => {
    it("should export account info in JSON format", async () => {
      ...
      expect((result as { content: { text: string }[] }).content[0].text).toContain("Account Export");
  ...
  describe("export-hashtag tool", () => {
    it("should export hashtag posts in JSON format", async () => {
      ...
      expect((result as { content: { text: string }[] }).content[0].text).toContain("Hashtag Export");
...
describe("formatPost helper function", () => {
  ...
  it("should handle posts with HTML content", async () => {
    ...
    const registeredTools = new Map<string, { handler: (...args: unknown[]) => Promise<unknown> }>();
    (mcpServer.registerTool as Mock).mockImplementation((name: string, config: unknown, handler: (...args: unknown[]) => Promise<unknown>) => {
