...
| `MCP_HTTP_ALLOWED_ORIGINS` | _empty_ | Comma-separated CORS origins. Default is empty (no origins allowed); set explicitly for browser clients. |
...
The `/health` HTTP endpoint returns a trivial liveness response (`\{"status":"ok"\}`) and does not require authentication. Use `HEALTH_CHECK_EXTERNAL_PROBE` to gate the optional outbound connectivity probe.

| Variable | Default | Description |
|----------|---------|-------------|
| `HEALTH_CHECK_EXTERNAL_PROBE` | `true` | Set to `false` to skip the outbound mastodon.social connectivity check. Useful for air-gapped environments. |
