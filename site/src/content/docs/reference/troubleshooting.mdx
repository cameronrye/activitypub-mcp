...
1. **Restart the server** to flush the in-memory actor cache (cache TTL also expires naturally; configure via `ACTOR_CACHE_TTL`).
...
1. **Restart the server** to flush in-memory caches, or shorten `CACHE_TTL` / `ACTOR_CACHE_TTL` in your env.
