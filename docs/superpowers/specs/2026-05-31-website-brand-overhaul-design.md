...
3. **Capabilities** — three figures with the **real build-derived counts**: **42 tools**, **15 resources**, **10 prompts** (see derivation below). Each figure links to its API reference page (`api/tools`, `api/resources`, `api/prompts`). No hardcoded numbers; the stale `53/10/11` must not appear anywhere.
...
- Reads the MCP registry source files. Based on the verified current source, registrations live in exactly five files: `src/mcp/tools.ts` (9 tool registrations), `src/mcp/tools-write.ts` (28 tool registrations), `src/mcp/capabilities.ts` (5 tool registrations), `src/mcp/resources.ts` (15 resource registrations), and `src/mcp/prompts.ts` (10 prompt registrations). The generator must scan **all** files under `src/` for `.registerTool` / `.registerResource` / `.registerPrompt` (do not hardcode the file list), so newly added registrations in any source file are picked up automatically — in particular it must not miss `tools-write.ts` or `capabilities.ts`.
- Extracts the counts and names of `.registerTool`, `.registerResource`, and `.registerPrompt` registrations (regex or AST).
...
**Resolved count discrepancy (surfaced explicitly):** the AST-based research report counted 37 tools / 10 resources / 5 prompts, while a direct `grep` of the source registry (confirmed in this repo) counts **42 `.registerTool`, 15 `.registerResource`, 10 `.registerPrompt`**. The grep matches the locked approved design and surface audit. The AST count undercounted (it missed the 28 write tools in `tools-write.ts`, the 5 tools in `capabilities.ts`, and several resource registrations). **Decision:** the build manifest generator is the single source of truth; it must reproduce **42 / 15 / 10** when run against the current source. If the generator and the grep ever disagree, the generator is fixed until it matches the actual registrations — no number is ever hand-entered into a page. The version badge (`v3.0.0`) is likewise derived from `package.json` at build time, never hardcoded.
...
Deliverables: install the three `@fontsource` packages; create `site/src/styles/fonts.css` (imported in `BaseLayout` before the no-FOUC script) and the token block (§2.4); rewrite the color/font sections of `main.css` to tokens; update dark-mode overrides to ink/paper (and add the `prefers-color-scheme` system fallback per §2.4); produce new `favicon.svg`, `logo.svg`, `logo-monochrome.svg`, and the inverse variant; regenerate `og-image.png` via the updated `generate-og-image.js`; set `theme-color` to vermilion; update README header; drop the Inter/Google Fonts link. Also create `scripts/generate-registry-manifest.js`, run it to produce and commit `site/src/data/registry-manifest.json` (must read 42/15/10), and update the `build:site` script to invoke it first — so the manifest exists before any phase that consumes it.
...
- **Count drift / wrong numbers on homepage.** *Risk:* re-hardcoding stale `53/10/11`. *Mitigation:* build-time manifest is the only source; the build shows a mismatch if the manifest disagrees with the grep (42/15/10). No page hardcodes counts or version.
- **Manifest generator undercounts.** *Risk:* the generator misses files (as the AST research did, 37/10/5 — it skipped `tools-write.ts` and `capabilities.ts`). *Mitigation:* generator scans all of `src/`, not a fixed file list, and must reproduce the verified grep counts; treat any divergence as a generator bug to fix before ship.
...
- [ ] `scripts/generate-registry-manifest.js` exists, scans all of `src/`, and produces `site/src/data/registry-manifest.json` with **tools: 42, resources: 15, prompts: 10**, matching the source grep; the homepage and API pages render these numbers from the JSON (no hardcoded counts anywhere).
